...
{
    /// <summary>
    /// Throws an <see cref="DomainValidationException" /> or a custom <see cref="Exception" /> if string <paramref name="input"/> is too short.
    /// </summary>
        ...
        Func<Exception>? exceptionCreator = null)
    {
        ...
        if (input.Length < minLength)
        {
            ...
            throw exception ?? new DomainValidationException(message ?? $"Input {parameterName} with length {input.Length} is too short. Minimum length is {minLength}.",
                parameterName);
        }
    ...
    /// <summary>
    /// Throws an <see cref="DomainValidationException" /> or a custom <see cref="Exception" /> if string <paramref name="input"/> is too long.
    /// </summary>
    /// <param name="guardClause"></param>
    /// <param name="input"></param>
        ...
        Func<Exception>? exceptionCreator = null)
    {
        ...
        if (input.Length > maxLength)
        {
            ...
            throw exception ?? new DomainValidationException(message ?? $"Input {parameterName} with length {input.Length} is too long. Maximum length is {maxLength}.",
                parameterName);
        }
