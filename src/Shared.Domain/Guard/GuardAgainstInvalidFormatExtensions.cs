...
{
    /// <summary>
    /// Throws an <see cref="DomainValidationException" /> or a custom <see cref="Exception" /> if  <paramref name="input"/> doesn't match the <paramref name="regexPattern"/>.
    /// </summary>
        ...
        Func<Exception>? exceptionCreator = null)
    {
        ...
        if (!m.Success || input != m.Value)
        {
            ...
            throw exception ?? new DomainValidationException(message ?? $"Input {parameterName} was not in required format", parameterName);
        }
        ...
        Func<Exception>? exceptionCreator = null)
    {
        if (!predicate(input))
        {
            ...
            throw exception ?? new DomainValidationException(message ?? $"Input {parameterName} did not satisfy the options", parameterName);
        }
