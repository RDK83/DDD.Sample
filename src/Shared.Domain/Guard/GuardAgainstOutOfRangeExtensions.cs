...
{
    /// <summary>
    /// Throws an <see cref="DomainValidationException" /> or a custom <see cref="Exception" /> if string <paramref name="input"/> length is out of range.
    /// </summary>
    ...
    /// <summary>
    /// Throws an <see cref="DomainValidationException" /> or a custom <see cref="Exception" /> if <paramref name="input"/> is not a valid enum value.
    /// </summary>
        ...
        Func<Exception>? exceptionCreator = null) where T : struct, Enum
    {
        if (!Enum.IsDefined(typeof(T), input))
        {
            ...
            {
                throw exception ?? new DomainValidationException(message ?? $"Invalid enum {parameterName}.", parameterName);
            }

            throw exception ?? new DomainValidationException(message, parameterName);
        }
        ...
        Func<Exception>? exceptionCreator = null) where T : struct, Enum
    {
        if (!Enum.IsDefined(typeof(T), input))
        {
            ...
            throw exception ?? new DomainValidationException(message);
        }
        ...
        Func<Exception>? exceptionCreator = null) where T : IComparable, IComparable<T>
    {
        if (rangeFrom.CompareTo(rangeTo) > 0)
        {
            throw exceptionCreator?.Invoke() ??
                  new DomainValidationException(message ?? $"{nameof(rangeFrom)} should be less or equal than {nameof(rangeTo)}", parameterName);
        }
        ...
        string? message = null, Func<Exception>? exceptionCreator = null)
    {
        ...
        return NullOrOutOfRangeInternal(guardClause, input, parameterName, new DateTime(sqlMinDateTicks), new DateTime(sqlMaxDateTicks), message, exceptionCreator);
    }
    ...
    {
        guardClause.Null(input, nameof(input), exceptionCreator: exceptionCreator);
        return NullOrOutOfRangeInternal(guardClause, input.Value, parameterName, rangeFrom, rangeTo, message, exceptionCreator);
    }
        ...
        Func<Exception>? exceptionCreator = null) where T : IComparable<T>?
    {
        ...
        if (rangeFrom.CompareTo(rangeTo) > 0)
        {
            throw new DomainValidationException(message ?? $"{nameof(rangeFrom)} should be less or equal than {nameof(rangeTo)}", parameterName);
        }
        ...
        if (input.CompareTo(rangeFrom) < 0 || input.CompareTo(rangeTo) > 0)
        {
            ...
            if (string.IsNullOrEmpty(message))
            {
                throw exception ?? new DomainValidationException(parameterName, $"Input {parameterName} was out of range");
            }
