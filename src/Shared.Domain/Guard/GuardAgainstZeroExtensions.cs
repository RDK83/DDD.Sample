...
{
    /// <summary>
    /// Throws an <see cref="DomainValidationException" /> or a custom <see cref="Exception" /> if <paramref name="input" /> is zero.
    /// </summary>
        ...
        Func<Exception>? exceptionCreator = null) where T : struct
    {
        if (EqualityComparer<T>.Default.Equals(input, default(T)))
        {
            ...
            throw exception ?? new DomainValidationException(message ?? $"Required input {parameterName} cannot be zero.", parameterName);
        }
