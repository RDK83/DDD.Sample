...
{
    /// <summary>
    /// Throws an <see cref="DomainValidationException" /> if <paramref name="func"/> evaluates to false for given <paramref name="input"/> 
    /// </summary>
    ...
    {
        if (!func(input)) throw new DomainValidationException(message);
    ...
    public static async Task<T> AgainstExpressionAsync<T>(this IGuardClause guardClause,
        ...
        string message) where T : struct
    {
        if (!await func(input)) throw new DomainValidationException(message);
    ...
    {
        if (!func(input)) throw new DomainValidationException(message, paramName);
