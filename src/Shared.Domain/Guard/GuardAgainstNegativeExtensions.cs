...
{
    /// <summary>
    /// Throws an <see cref="DomainValidationException" /> or a custom <see cref="Exception" /> if <paramref name="input"/> is negative.
    /// </summary>
        ...
        string? message = null, Func<Exception>? exceptionCreator = null) where T : struct, IComparable
    {
        if (input.CompareTo(default(T)) < 0)
        {
            ...
            throw exception ?? new DomainValidationException(message ?? $"Required input {parameterName} cannot be negative.", parameterName!);
        }
    ...
    /// <summary>
    /// Throws an <see cref="DomainValidationException"/> or a custom <see cref="Exception" /> if <paramref name="input"/> is negative or zero.
    /// </summary>
    /// <param name="guardClause"></param>
    /// <param name="input"></param>
    /// <param name="parameterName"></param>
        ...
        Func<Exception>? exceptionCreator = null) where T : struct, IComparable
    {
        if (input.CompareTo(default(T)) <= 0)
        {
            ...
            throw exception ?? new DomainValidationException(message ?? $"Required input {parameterName} cannot be zero or negative.", parameterName!);
        }
